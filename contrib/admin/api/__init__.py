...
from ..core.actions.tokens import ScopeTokenService
...
class AdminAPI:
    ...
    def __init__(self, adapter: BaseAdapter | None = None) -> None:
        ...
        self.router.get(
            self._relative(self.LOOKUP_PATH), name="admin.api.lookup"
        )(self.api_lookup)
        self.router.get(
    ...
    async def api_lookup(
        ...
        admin_site = request.app.state.admin_site
        admin = self._get_admin(admin_site, app_label, model_name)
        if field not in admin.get_autocomplete_fields():
            raise HTTPException(status_code=404)
        md = self.adapter.get_model_descriptor(admin.model)
        fd = md.fields_map.get(field)
        if not fd or not fd.relation or fd.relation.kind != "fk":
            raise HTTPException(status_code=404)
        rel_model = self.adapter.get_model(fd.relation.target)
        rel_md = self.adapter.get_model_descriptor(rel_model)
        search_fields = admin.get_search_fields(rel_md)
        qs = admin.get_autocomplete_queryset(fd, q, search_fields=search_fields)
        qs = admin.apply_row_level_security(qs, user)
        try:
            per_page = await system_config.get(SettingsKey.DEFAULT_PER_PAGE)
        except KeyError:  # minimal setup in tests
            per_page = 20
        page = max(1, int(page))
        offset = (page - 1) * per_page
        qs = self.adapter.limit(qs, per_page)
        qs = self.adapter.offset(qs, offset)
        objs = await self.adapter.fetch_all(qs)
        return [
            {"id": getattr(o, rel_md.pk_attr), "title": str(o)} for o in objs
        ]
