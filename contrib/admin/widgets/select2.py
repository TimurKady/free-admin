...
class Select2Widget(
    ...
    """Select2-поле для FK. Делает правильный ajax.url и ленивую подгрузку."""
    ...
    def base_url(self) -> str:
        """Базовый префикс lookup API: '/<ADMIN>/<API>/lookup'."""
    ...
    def _get_owner_app_model(self) -> tuple[Optional[str], Optional[str]]:
        """Возвращает пару app/model владельца поля из ``ctx.descriptor``."""
    ...
    def _resolve_lookup_url(self) -> str:
        """
        Собираем ajax.url:
          1) meta['lookup_path'] или self.lookup_path (приоритет)
          2) /<admin>/<api>/lookup/<owner_app>/<owner_model>/<field>
          3) /<admin>/<api>/lookup/<field> (последний шанс)
        """
    ...
    def _limit_prefetched_choices(
        ...
        """Обрезаем список до sample_size, но сохраняем выбранное значение."""
    ...
    async def prefetch(self) -> None:
        """Подготавливаем choices_map и обрезаем до prefetch_sample_size."""
    ...
    def get_schema(self) -> Dict[str, Any]:
        """Возвращает JSON Schema для FK/M2M без super().get_schema().  :contentReference[oaicite:0]{index=0}"""
