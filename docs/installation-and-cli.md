...
### Wiring the aggregated router

The scaffold also generates `config/routers.py`, which exposes a convenience helper—typically named `get_admin_router()`—that wraps the router class produced by the CLI. Import this helper during application bootstrap to keep the main module focused on startup concerns:
...
from config.orm import init_orm
from config.routers import get_admin_router
...
By centralising the router imports in `config/routers.py`, the scaffold keeps the application entrypoint clean and ensures every admin route generated by the CLI is automatically exposed.
